...
class PairedTrainingPipeline:
    ...
    def __init__(self):
        ...
        # Load MiDaS model (using most compatible version)
        model_path = "MiDaS/dpt_hybrid_384.pt"
        if not Path(model_path).exists():
            print("❌ Download MiDaS weights first!")
            sys.exit(1)
            
        self.model, self.transform, self.net_w, self.net_h = load_model(
            self.device, model_path, "dpt_hybrid_384", optimize=False, height=None, square=False
        )
    ...
    def extract_exif_metadata(self, image_path):
        ...
        cv2.putText(comparison, iphone_label, (10, 30), font, 1, (255, 255, 255), 2)
        cv2.putText(comparison, sony_label, (iphone_resized.shape[1] + 10, 30), font, 1, (255, 255, 255), 2)
        cv2.putText(comparison, "iPhone Depth", (10, iphone_resized.shape[0] + 30), font, 1, (255, 255, 255), 2)
        cv2.putText(comparison, "Sony Depth", (iphone_resized.shape[1] + 10, iphone_resized.shape[0] + 30), font, 1, (255, 255, 255), 2)
